...
st.write(
    "Discover the best restaurants near you based on your preferences. Foodie is currently in testing, so please bear with bugs. "
    "Foodie works best on laptops. Made by Hritish."
)

st.sidebar.header("🔍 Search Settings")

# **City or Address Selection**
...
if selected_city != "N/A":
    ...
    disable_address_input = True  # Hide address input
else:
...
if selected_city != "N/A":
    cuisine = northeast_city_food_map[selected_city][0]  # Default to city's iconic dish
...
def get_restaurants():
    ...
    for b in businesses:
        business_categories = {cat["alias"] for cat in b.get("categories", [])}

        if business_categories.intersection(blocked_categories):
            continue  # Skip these businesses
